...
if($_GET['getOrder'])
{
    $sql = "SELECT orders_id, orders_subtotal, orders_collectTime, payment_method, payment_dateTime, points_received, points_redeemed FROM orders JOIN payment USING(orders_id) WHERE user_id = '$user_id' AND orders_status IS NOT NULL LIMIT 10";
    $result = $connnectDB ->query($sql);
    $result = $result->fetch_all(MYSQLI_ASSOC);
